...
#Mas saiba que esta funcao ja existe. Tambem para mediana, max e min
mean(horas)
median(horas)
max(horas)
min(horas)
...
#Como primeiro exemplo vamos gerar uma tabela hipotetica de notas dos alunos de uma disciplina. Perceba a estrutura do argumento
notas<- data.frame(alunos=c("Maria","Joao","Pedro","Alice"), notas=c(10,10,10,10))

#Para verificar
View(notas)

#Se quiser saber quantas colunas e linhas tem seu dataframe
ncol(notas)
nrow(notas)

#Agora iremos gerar uma tabela que realmente sera pratica para voces, o cronograma do nosso curso

cronograma <- data.frame(Dia=c("18", "21", "22", "25", "28", "29","2","5","6","9"), Mes=c("Setembro","Setembro","Setembro","Setembro","Setembro","Setembro","Outubro","Outubro","Outubro","Outubro"), 
                         Tema=c("Fundamentos de Biogeografia e evolução do pensamento biogeográfico", "Introducao a linguagem R", "Biogeografia da dispersao e Biogeografia de Ilhas", "Obtencao de dados de ocorrencia GBIF", "Distribuicao deografica e Biogeografia da America do Sul", "Limpeza de dados e obtencao de informacoes bioclimaticas", "Distribuicao geografica e capacidade de dispersao pontencial, correlacoes, mapas com dados de rasters","Drivers de dispersao e distribuicao", "Desenvolvimento do trabalho", "Avaliacao final"))
...
#Para verificar a estrutura do dataframe
str(cronograma)

#Tente somar os dias. Neste caso o uso de $ se refere a indicacao de coluna e o uso de [] se refere a indicacao do numero da linha na qual se encontra a coluna. Vou te explicar melhor daqui a pouquinho.
cronograma$Dia
sum(cronograma$Dia[1]+cronograma$Dia[2])

#Neste caso os numeros estao como caracteres e nao podem ser somados. Se quiser trocar estes numeros para numeros somaveis
cronograma$Dia<-as.numeric(cronograma$Dia)
str(cronograma)
cronograma$Dia
sum(cronograma$Dia[1]+cronograma$Dia[2])
...
#Para salvar este cronograma como CSV. Nao esqueca de selecionar a pasta que voce esta trabalhando primeiro
setwd("/Users/xbaroc/Desktop/Disciplina Dispersão/Apostila prática_2023/Dados2023")
write.csv(cronograma, file= "cronograma.csv")

#Para abrir um document em .csv simples
cronograma2<-read.csv("/Users/xbaroc/Desktop/Disciplina Dispersão/Apostila prática_2023/Dados2023/cronograma.csv")
View(cronograma2)

#Contudo, existem diversas formas de abrir tabelas no R, mas aqui vamos aprender a funcao mais simples para abrir dados do GBIF, a read.tsv do pacote readr. Exemplos:


library(readr)

vriesea_gbif<-read_tsv("/Users/xbaroc/Desktop/Disciplina Dispersão/Apostila prática_2023/Dados2023/vriesea_occ_gbif.csv",quote="")
...
#Se quiser trocar um argumento comum em varias linhas, como no caso de Liliopsida para Monocotiledonea na coluna de classe, utilize
...
#So funciona para argumentos numericos. Neste caso tive que excluir os NA.
...
#Outras formas de filtro utilizando os argumentos do dplyr, neste caso para elevacao maior que 1000m
vriesea_elev_1000 = vriesea_gbif %>% filter(elevation>1000)

#Ou ainda usando duas condicoes. Vriesea oliganta e Vriesea gelatinosa ou acima de 1000 m
vriesea_ol_gel_1000 <- subset(vriesea_gbif, elevation > 1000 | species %in% c("Vriesea oligantha", "Vriesea gelatinosa"))
#Ou ainda usando duas condicoes. Vriesea oliganta e Vriesea gelatinosa e acima de 1000 m
vriesea2_ol_gel_1000 <- subset(vriesea_gbif, elevation > 1000 & species %in% c("Vriesea oligantha", "Vriesea gelatinosa"))
...
#Para encontrar a citacao dos pacotes utilizados e versao
citation("dplyr")
...
alunos <- data.frame(nome=c("Maria", "Joao", "Pedro","Alice"),salario=c(2100,2100,3100,3100), nivel=c("Mestrado", "Mestrado", "Doutorado", "Doutorado"))
View(alunos)
...
#Primeiro temos que instalar e abrir o rnaturalearthdata, o CoordinateCleaner nós ja temos

install.packages("rnaturalearthdata")
library(rnaturalearthdata)

############## Limpeza manual e usando o CoordinateCleaner ##########
#Vamos comecar chamando a nossa planilha base
...
frame <- read.csv("/Users/xbaroc/Desktop/Disciplina Dispersão/Disciplina JB/Vriesea_occ/vriesea_gbif3.csv", stringsAsFactors = F)
...
for (i in 1:length(list_species)){
  ...
  df_temp = cbind.data.frame(max = max(shp, na.rm = T),min = min(shp, na.rm = T), mean= mean(shp, na.rm = T), median=median(shp, na.rm = T))
...
#Utilizaremos a mesma logica do exemplo anterior. Como exemplo vamos utilizar a bio12

#Temos que chamar novamente a planilha que tem a informacao dos rasters
frame <- read.csv("/Users/xbaroc/Desktop/Disciplina Dispersão/Apostila prática/Script e dados/vriesea_gbif3_rasters.csv", stringsAsFactors = F)
long="decimalLongitude"
lat="decimalLatitude" 
species = "species" 
...
shp=df_temp
mean(df_temp$bio12)
median(df_temp$bio12)

frame$mean_bio12=NULL
frame$median_bio12=NULL
...
for (i in 1:length(list_species)){
  ...
  df_temp = frame[frame[[species]]==sp,]
  shp=df_temp
  df_temp = cbind.data.frame(mean_bio12 = mean(shp$bio12, na.rm=T),median_bio12 = median(shp$bio12, na.rm=T),max_bio12=max(shp$bio12, na.rm=T),min_bio12=min(shp$bio12, na.rm=T))
  df_final = rbind(df_final,df_temp)
}
View(df_final)
...
#Como exemplo realizaremos o teste apenas para a bio12

shapiro.test(as.numeric(frame4$mean))
shapiro.test(as.numeric(frame4$mean_bio12))
...
bio12<-ggscatter(frame4, x = "mean", y = "mean_bio12", 
                add = "reg.line", conf.int = TRUE, 
                cor.coef = TRUE, cor.method = "spearman",
                xlab = "Dispersal capacity (km)", ylab = "Precipitation (mm)\n", alpha=0.1)
bio12

#Para adicionar um histograma marginal para compreender a ditribuicao dos dados
bio12b<-ggMarginal(bio12, type = "histogram",margins = 'y')
bio12b
...
boxplot(frame4$mean_bio12)
boxplot(frame4$mean_bio12, plot=FALSE)$out
...
outliers <- boxplot(frame5$mean_bio12, plot=FALSE)$out
frame5<- frame5[-which(frame5$mean_bio12 %in% outliers),]

#Verificando novamente
bio12<-ggscatter(frame5, x = "mean", y = "mean_bio12", 
                add = "reg.line", conf.int = TRUE, 
                cor.coef = TRUE, cor.method = "spearman",
                xlab = "Dispersal capacity (km)", ylab = "Temperature (C°)\n", alpha=0.1)
bio12

bio12b<ggMarginal(bio1, type = "histogram",margins = 'y')
bio12b

#Alem disso, se voces tiver valores muito diferentes entre si nas variaveis, voce podera fazer log10. Opcional!

frame4$log10_mean_bio12<-log10(frame4$mean_bio12)
frame4$log10_mean<-log10(frame4$mean)

bio12<-ggscatter(frame4, x = "log10_mean", y = "log10_mean_bio12", 
                add = "reg.line", conf.int = TRUE, 
                cor.coef = TRUE, cor.method = "spearman",
...
pdf("correlation_prec.pdf",width = 10,height =5)
bio12
dev.off()


######### Mapas com dados de rasters ########
#Provavelmente voce percebeu que os dados extraidos se referem a soma das medias mensais, desta forma, para isto devemos dividir por 10 para ter um ideia de media anual
#Para isto deveremos utilizar nossa planilha com dados de coordenadas geograficas, nao a de especies.


frame <- read.csv("/Users/xbaroc/Desktop/Disciplina Dispersão/Apostila prática/Script e dados/vriesea_gbif3_rasters.csv", header=T)

#Para termos a media de temperatura mensal deveremos dividir por 10 o valor obtido dos rasters
frame$bio1<-frame$bio1 /10
...
g1<-ggplot() + 
  ...
  geom_point(data = frame, aes(x = decimalLongitude,  y = decimalLatitude, fill = bio1_10), shape=21,color='grey90', stroke=0.0001,size=1) +
  coord_fixed() + scale_fill_viridis_c(name = "Precipitation (mm)\n", direction = -1, option = "inferno")+ theme_bw() + 
  theme(axis.title = element_blank())
g1
...
g2<-ggplot() + 
  geom_map(data = world.inp, map = world.inp, aes(x = long, y = lat,  map_id = region),  color = "gray", fill = "grey80")  +
  geom_point(data = frame, aes(x = decimalLongitude,  y = decimalLatitude, fill = bio1_10), shape=21,color='grey90', stroke=0.0001,size=2) +
  coord_fixed() + scale_fill_viridis_c(name = "Precipitation (mm)\n", direction = -1, option = "inferno")+ theme_bw() + 
...
pdf("map_prec.pdf",width = 10,height =10)
g2
dev.off()

#Outra forma e plotar os pontos sobre os mapas rasterizados, desta forma:
install.packages("elevatr")
library(elevatr)
e<-extent(c(-180,180,-90,90))
#e<-extent(c(-100,-25,-40,30))
elev <- get_elev_raster(raster(e, vals = 0), z = 3, prj = "+proj=longlat +datum=WGS84 +no_defs")
extent(elev)
plot_data <- raster::extract(elev, raster::extent(elev), buffer = 10)
plot_df <- data.frame(raster::xyFromCell(elev, 1:ncell(elev)), value = plot_data)
elev2<-fortify(plot_df, maxpixels = 1000000000)

g5 <- ggplot() +
  geom_raster(data = elev2, aes(x = x, y = y, fill = value)) + 
  geom_point(data = frame, aes(x = decimalLongitude, y = decimalLatitude), colour = "red", size = 0.3)+
  scale_fill_gradient(name = "Elevation (m)\n", low = "white", high = "darkblue", limits=c(0,5000)) +
